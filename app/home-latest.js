// 메인 페이지 "시공사진" 영역을 Supabase 갤러리 최신 4건으로 교체 (게시물이 없으면 기존 정적 사진 유지)
import { sb } from './supabase.js';
import { esc, youtubeId } from './ui.js';
  ...
  try {
    ...
    if (!error && data && data.length) {
      ...
      list.innerHTML = data.map((p) => {
        ...
        const href = `app/gallery.html?id=${p.id}`;
        let cover;
        if (!m) cover = `<img src="theme/home/img/main/main_section_2.jpg" alt="">`;
        else if (m.media_type === 'image') cover = `<img src="${esc(m.url)}" alt="${esc(p.title)}">`;
        ...
        else cover = `<video src="${esc(m.url)}#t=0.5" muted preload="metadata" style="width:100%;height:100%;object-fit:cover"></video>`;
